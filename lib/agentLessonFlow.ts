...
export const intensiveLanguageTeachingFlowPrompt = [
  ...
  "同一词族在前页已经精讲过时，当前页再次出现最多一句简短回顾它在当前句中的意思，然后立即进入新词、当前句结构或应用。",
  "禁止重复词性、完整词义列表、常见搭配、发音拆解和生活例句；词形变化也算同一词族，例如 spot/spots、come/came、child/children。",
  "重点句采用儿童习得方式：先结合故事语境说清句意，每个可见跨页最多讲一个最有用的语言规律，再用一到两个短例句或替换应用帮助孩子自然理解。",
  "只使用少量儿童能理解的说法，例如‘这里用 came，表示事情已经发生了’或‘把 doctor 换成 teacher，就能表达另一个人来了’。没有必要的语法点时重点讲词汇、固定搭配和故事表达，不强行加入语法。",
  "禁止讲主谓宾、宾语补足语、时态定义、动词变位表等抽象理论，不要求孩子背规则。语法问题只能采用简单选择、替换一个词或理解原句的形式。",
  ...
  "每个可见跨页最多提出一个问题，必须在左右页全部讲完后再问。问题只能考查当前原文中的语境词义、简单表达规律、替换一个词完成新句子或原文明确信息。",
